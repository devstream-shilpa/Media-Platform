...
terraform {
  ...
  #   key    = "media-platform/terraform.tfstate"
  #   region = "us-east-1"
  # }
}
...
resource "aws_launch_template" "app" {
  ...
  user_data = base64encode(templatefile("${path.module}/user_data.sh", {
    ...
    sqs_queue_url  = aws_sqs_queue.media_processing.url
  }))
...
resource "aws_sqs_queue" "media_processing_dlq" {
  name = "${var.project_name}-media-processing-dlq"
...
resource "aws_sqs_queue_redrive_policy" "media_processing" {
